...
class Instagram:
    ...
    def __init__(self, sleep_between_requests=0):
        ...
        m = 0
        self.fname = "comments.csv"
        while os.path.exists(self.fname):
            m += 1
            self.fname = "comments" + "(" + str(m) + ")" + ".csv"
    ...
    def get_media_comments_by_id(self, media_id, count, max_id=None):
        """
        :param media_id: media id
    ...
    def excel_exporter(names, comments, self):
        try:
            temp = {}                    
            temp_names = []
            temp_comments = []
            if os.path.isfile(self.fname):
                saved = pd.read_csv(self.fname)
            ...
            temp.update({'a - username': temp_names, 'b - comment': temp_comments})
    ...
    def get_media_comments_by_code(self, code, count, max_id=''):
        ...
        time.sleep(2)
        print("\n------------------------------------------------------\n")
        print("\r[!] Publicacao encontrada!")
        print("\n------------------------------------------------------\n")
        while index < count:
            print("\r[!] Buscando comentarios...")
            try: 
                #2362088081258894915
                ...
                if not response.status_code == Instagram.HTTP_OK:
                    print("\n------------------------------------------------------\n")
                    print("\r[!] Extracao bloqueada pelo instagram\nAguardando 5 minutos para continuar")
                    print("\n------------------------------------------------------\n")
                    print("\n")
                    print("\n------------------------------------------------------\n")
                    print("\r[!] Por favor, envie o codigo abaixo ao desenvolvedor")
                    print("\n------------------------------------------------------\n")
                    print("\n")
                ...
                print("\n------------------------------------------------------\n")
                print("\r[!] Extracao parada pelo usuario. Encerrando processo de extracao dos comentarios")
                print("\n------------------------------------------------------\n")
                break
            except:
                print("\n------------------------------------------------------\n")
                print("\r[!] Extracao bloqueada por algum erro desconhecido\nAguardando 5 minutos para continuar")
                print("\n------------------------------------------------------\n")
                print("\n")
                time.sleep(300)
                continue
            ...
            #extraindo dados já obtidos
            for commentArray in nodes:
                comments.append(str(commentArray['node']['text']))
                users.append(str(commentArray['node']['owner']['username']))
                index += 1
                index_code += 1

                # all responses from this comment in for loop
                for commentResponseArray in commentArray['node']['edge_threaded_comments']['edges']:
                    comments.append(str(commentResponseArray['node']['text']))
                    users.append(str(commentResponseArray['node']['owner']['username']))
                    index += 1
                    index_code += 1

            print("\r------------------------")
            print("\r[!] Comentarios obtidos neste request => " + str(index_code))
            print("\r[!] Total de comentarios obtidos => " + str(index))
            print("\r[!] Total esperado a ser obtido => " + str(count))
            if has_previous == True:
                print("\r[!] Ainda ha comentarios para receber, segundo o instagram!")
                print("\r[!] Status => " + str(has_previous))
            if not has_previous == True:
                print("\n[!]\n")
                print("\rSegundo o instagram, nao ha mais comentarios para receber.\nA extracao continuara ate que seja parada pelo usuario!")
                print("\rPara forcar a finalizacao da extracao, pressione CTRL + C")
                print("\rStatus => " + str(has_previous))
                print("\n[!]\n")

            print("[!] Salvando os " + str(index_code) + " comentarios")

            users.pop(0)
            comments.pop(0)
            Instagram.excel_exporter(users, comments, self)
            ...
            print("\r[!] Quantidade de comentarios na publicacao atualmente => " + str(number_of_comments))
            ...
            if len(nodes) == 0:
                ...
                print("\rTotal de tentativas ate agora => " + str(tentativa) )

                print("\rAguardando 5 minutos para tentar novamente")
                time.sleep(300)
                continue
              

        print("\r[!] Extracao finalizada!")
        data = {}

        data['comments'] = int(index)
        data['csv_file'] = str(self.fname)
        return data
